...
# Replace the chat_with_openai function with this
def chat_with_local_llm(user_input, image_path=None, current_iteration=None, max_iterations=None):
    global conversation_history, automode
    
    # Create the prompt
    prompt = get_prompt(user_input, tools)
    ...
    if '<<function>>' in assistant_response:
        function_calls = assistant_response.split('<<function>>')[1:]
    
    for function_call in function_calls:
        try:
            function_data = json.loads(function_call)
            tool_name = function_data['name']
            tool_input = function_data['arguments']
            ...
            # Add the result to the conversation and get a new response
            new_prompt = get_prompt(f"Tool {tool_name} returned: {result}. Please continue.", tools)
            new_output = llm(new_prompt, max_tokens=40000, stop=["<|EOT|>"], echo=False)
            assistant_response += "\n" + new_output['choices'][0]['text']
        except json.JSONDecodeError:
            print_colored(f"Error parsing function call: {function_call}", TOOL_COLOR)
...
def main():
    ...
    while True:
        ...
        if user_input.lower() == 'image':
            ...
            response, _ = chat_with_local_llm(user_input)
            process_and_display_response(response)
